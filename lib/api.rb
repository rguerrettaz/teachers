  ...
  module ClassMethods  
      def call_api(zip,state,city) 
        if zip.blank? && city.blank?
            query = JSON.parse(open("http://api.education.com/service/service.php?resf=json&f=schoolSearch&key=fee0d93eb35e6b061392922ddfc720bd&sn=sf&v=4&state=#{state}").read)
        elsif zip.blank?
            query = JSON.parse(open("http://api.education.com/service/service.php?resf=json&f=schoolSearch&key=fee0d93eb35e6b061392922ddfc720bd&sn=sf&v=4&state=#{state}&city=#{URI::encode(city).downcase}").read)
        elsif city.blank?
            query = JSON.parse(open("http://api.education.com/service/service.php?resf=json&f=schoolSearch&key=fee0d93eb35e6b061392922ddfc720bd&sn=sf&v=4&state=#{state}&zip=#{zip.to_i}").read)
        else 
            query = JSON.parse(open("http://api.education.com/service/service.php?resf=json&f=schoolSearch&key=fee0d93eb35e6b061392922ddfc720bd&sn=sf&v=4&state=#{state}&zip=#{zip.to_i}&city=#{URI::encode(city).downcase}").read)
        end
          p "I'm in the API!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
          p query
